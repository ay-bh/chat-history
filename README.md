...
CLI metadata discovery supports schema version 1 (verified against Cursor Agent 2026.08.25). Both `meta.json` and a nonempty `store.db` are required. CLI-only rows are labeled `[metadata only]` in the session list: title search, `find`, and `resume` work, but `inspect`, `view`, and `export` report that a transcript is unavailable. An existing readable transcript takes precedence. The CLI's protobuf blob store is not decoded, so CLI-only message search/export remains unsupported until a transcript is available. Resume also requires the recorded workspace to exist and rejects explicitly empty conversations.
